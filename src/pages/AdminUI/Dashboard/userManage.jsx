...
export default function UserManage() {
  ...
  const handleSearch = async (searchTerm) => {
    ...
    try {
      const res = await adminApi.UsersList(searchTerm.toString());
  ...
  const BlockUnblock = async (value, id) => {
    ...
    try {
      const res = await adminApi.UserBlockUnBlock(data, id);

      if (res.status === 200) {
        setUserDetails((prevGuideDetails) =>
    ...
    <>
      ...
      <Card className="h-screen w-full overflow-scroll">
        <table className="w-full min-w-max table-auto text-left">
          ...
          <tbody>
            {(Search ? filteredUserList : userDetails).length === 0 ? (
              <tr>
              ...
              (Search ? filteredUserList : userDetails).map(
                (
